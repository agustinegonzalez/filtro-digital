...
FS_REAL = 100000.0       # valor por defecto (se actualiza con trama)
VREF = 3.3
ADC_RES = 4095.0

PAUSADO = False

# Paleta de colores agradable estilo osciloscopio
COLOR_IN = "#4FA3F7"     # azul pastel
COLOR_OUT = "#FF3366"    # rosa rojizo
GRID_COLOR = "#AAAAAA"
BG_COLOR = "#1E1E1E"
...
def leer_serial():
    ...
    while True:
        try:
            ...
            # Protección contra buffer infinito
            if len(rx_buffer) > 20000:
                rx_buffer.clear()
                ser.reset_input_buffer()
                continue

            # Procesar paquetes consecutivos
            while len(rx_buffer) >= 12:
                ...
                fs_val, n_samples, seq = struct.unpack(
                    "<IHH", rx_buffer[4:12]
                )
                ...
                if rx_buffer[packet_size-4:packet_size] == MAGIC:
                    ...
                    FS_FROM_PACKET = fs_val  # actualizar automáticamente
